...
def extrair_blocos_linha(linha):
    ...
    """
    # Expressão regular ajustada para ser mais flexível.
    # Agora a parte de "horas_ou_qtd" é opcional, permitindo extrair itens
    # que só têm código, descrição e valor (ex: "PRODUÇÃO").
    pad = re.compile(
        r"(\d{1,4})\s+([A-Z0-9 .ÁÉÍÓÚÂÊÔÃÕÇ/%-]+?)\s+(?:(\d{1,3}(?:,\d{2})?)\s+)?(\d{1,3}(?:\.\d{3})*,\d{2})"
    )
    itens = []
...
if __name__ == "__main__":
    ...
    with open("folha_completa_centrais.json", "w", encoding="utf-8") as f:
        json.dump(resultado, f, ensure_ascii=False, indent=4)

    print("OK: folha_completa_centrias.json gerado com proventos, descontos e totais")
